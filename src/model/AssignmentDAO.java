...
public class AssignmentDAO {
	...
	public List<Assignment> getAllAssignments(String teamID) {
		...
		try {
			...
			while (rs.next()) {
				Assignment ass = new Assignment(rs.getInt("MaPhanCong"), rs.getString("MaQuyTrinh"),
	...
	public boolean createAssignment(Assignment assignment) {
		try {
			...
			stmt.setInt(4, assignment.getDetailProductionID());
	...
	public Assignment getAssignmentByID(String assignmentID) {
		...
		try {
			...
			if (!rs.next())
				...
				assignment = new Assignment(rs.getInt("MaPhanCong"), rs.getString("MaQuyTrinh"),
						rs.getString("MaNhanVien"), rs.getTimestamp("NgayThamGia").toLocalDateTime().toLocalDate());
			}
		} catch (SQLException e) {
			e.printStackTrace();
